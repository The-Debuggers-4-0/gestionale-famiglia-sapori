 ...
 */
public class CucinaController implements Initializable { // Trigger recompile
    ...
    void loadComande() {
        try {
            List<Comanda> comande = comandaDAO.getComandeByStatoAndTipo(Comanda.STATO_IN_ATTESA, "Cucina");
            comande.addAll(comandaDAO.getComandeByStatoAndTipo(Comanda.STATO_IN_PREPARAZIONE, "Cucina"));
            ...
            for (Comanda c : comande) {
                if (!activeCards.containsKey(c.getId())) {
                    ...
                    // Per ora ricreiamo solo se necessario, ma qui assumiamo che la card gestisca
                    // il suo stato
            ...
            if (activeCards.isEmpty() && ordersContainer.getChildren().isEmpty()) {
                // Gestione label vuota se necessario, ma con activeCards è più complesso
                // gestirlo misto
                ...
                if (comande.isEmpty()) {
                    ...
                    ordersContainer.getChildren()
                            .removeIf(node -> node instanceof Label && ((Label) node).getText().startsWith("Nessuna"));
                }
            }
        } catch (SQLException e) {
    ...
    private VBox createComandaCard(Comanda c) {
        ...
        card.setStyle(
                "-fx-background-color: #34495e; -fx-background-radius: 10; -fx-padding: 15; -fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.3), 10, 0, 0, 5);");
    ...
    private void updateStato(Comanda c, String nuovoStato) {
        try {
            ...
            // Non rimuovere manualmente da activeCards, lascia che loadComande gestisca la
            // sincronizzazione
            loadComande();
        } catch (SQLException e) {
